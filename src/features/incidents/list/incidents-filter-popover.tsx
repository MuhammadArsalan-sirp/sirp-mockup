...
const FILTER_GROUPS: FilterCategory[][] = [
  // State
  [
    { id: "recordType", label: "State", Icon: Layers, options: recordTypeOptions },
  ],
  // Triage
  [
    ...
    { id: "state", label: "Stage", Icon: MoreHorizontal, options: stateOptions },
  ],
  // Assignment
  [
...
function useActiveFilterCount(table: Table<Incident>): number {
  return table.getState().columnFilters.length
}

// ─── Sub-panel: options for a hovered category ───────────────────────────────
...
type Props = { table: Table<Incident> }

export function IncidentsFilterPopover({ table }: Props) {
  ...
  const hoveredCategory =
    hovered ??
    // keep showing last hovered while user moves between panels
    null
  ...
  return (
    ...
    >
      ...
      >
        <div className="flex">
          {/* Sub-panel — appears to the left when a category is hovered */}
          {hoveredCategory && (
