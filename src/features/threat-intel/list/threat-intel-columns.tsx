...
function StatusPill({ value }: { value: ThreatIntel["status"] }) {
...
> = {
  pending: { label: "Pending", tone: "var(--muted-foreground)", icon: Clock },
  case: { label: "Case", tone: "var(--chart-2)", icon: Briefcase },
  finish: { label: "Finish", tone: "var(--success)", icon: CheckCircle2 },
}

function StatePill({ value }: { value: ThreatIntelState }) {
...
function UserCell({
  ...
  const sizeCls = size === "sm" ? "size-7 text-[11px]" : "size-8 text-xs"
  return (
...
export const threatIntelColumnLabels: Record<string, string> = {
  id: "ID",
  sourceId: "Source ID",
  title: "Intel",
  severity: "Severity",
...
export const threatIntelColumns: ColumnDef<ThreatIntel>[] = [
  {
    id: "select",
    header: ({ table }) => (
      <Checkbox
  ...
  },
  {
    accessorKey: "title",
    header: ({ column }) => (
      <DataTableColumnHeader column={column} title="Intel" />
    ),
    cell: ({ row }) => (
  ...
  // ─── Additional columns (all hidden by default) ───────────────────────────────
  {
