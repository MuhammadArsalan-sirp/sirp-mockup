...
import type { ThreatIntel, ThreatIntelState, ThreatIntelStatus, Priority, Severity } from "@/data/threat-intel"
import type { BoardField, BoardGroupByField, BoardViewConfig, GroupByField } from "./threat-intel-view-types"
import { buildGroups, type GroupMeta } from "./threat-intel-feed"
...
const stageTone: Record<ThreatIntelState, string> = {
...
const LANES_BY_GROUP: Record<BoardGroupByField, Lane[]> = {
  recordType: [
    { key: "pending", label: "Pending", tone: "var(--muted-foreground)", Icon: Clock },
    { key: "case", label: "Case", tone: "var(--chart-2)", Icon: Briefcase },
    { key: "finish", label: "Finish", tone: "var(--success)", Icon: CheckCircle2 },
  ],
  state: [
    { key: "pending", label: "Pending", tone: "var(--muted-foreground)", Icon: Clock },
    { key: "case", label: "Case", tone: "var(--chart-2)", Icon: Briefcase },
...
function getLaneKey(inc: ThreatIntel, groupBy: BoardGroupByField): string {
  switch (groupBy) {
...
function renderCardField(field: BoardField, inc: ThreatIntel): React.ReactNode {
  switch (field) {
    case "severity": {
...
export function ThreatIntelBoard({ table, config }: Props) {
                ...
                {items.length === 0 ? (
                  ...
                  buildGroups(items, subGroupBy, showEmptyGroups).map((sg) => (
                    <BoardSubGroup
                      ...
                      rows={sg.rows}
                      cardFields={cardFields}
                      onOpen={(id) => navigate(`/threat-intel/${id}`)}
                    />
                  ))
                ) : (
                  items.map((inc) => (
                    <BoardCard
                      ...
                      incident={inc}
                      cardFields={cardFields}
                      onOpen={() => navigate(`/threat-intel/${inc.id}`)}
                    />
                  ))
                )}
              </div>
